...
# bfs will be more easy to implement
class Queue:
    def __init__(self):
        self.Queue = []
        ...
        for movement in [(1,0), (-1,0), (0,1), (0,-1)]:
            ...
            if (is_in_map(new_pos, self.grid_dim)) and (grid[new_pos[0], new_pos[1]] in [1, 3]) :
                ...
                if new_pos == goal_pos:
                    ...
                    solution = self.backtrack_solution(curr_node=new_node)
...
if __name__ == "__main__":
  ...
  # parsing user input
  # example: python bfs_pathfinder.py --display=0 --maze_file=maze_0.csv
  ...
  start_pos = (num_rows-1 , 0)
  goal_pos = (0 , num_columns-1)
  ...
  if args.display == 1:
    ...
    # set the height and width of each block in the maze
    height = 8
    width = height 
    margin = 1
    ...
    while not done:
        ...
        # color the solution to magenta.
        if done == True:
            for pos in solution:
                grid[pos[0], pos[1]] = 5
                # color the start and end to right color.
            grid[-1, 0] = 2
            grid[0, -1] = 3
