...
def is_in_map(pos, grid_dim):
    ...
    x_in = (x <= max_x) & (x >= 0) # check x in map
    y_in = (y <= max_y) & (y >= 0) # check y in map
...
class Queue:
    ...
    def get_node_position(self):
        node = self.Queue[-1]
        # return the last element's pos in queue
...
class DFS:
    ...
    def backtrack_solution(self, curr_node):
        solution = []
        # until trace back to start_point
...
if __name__ == "__main__":
  ...
  if args.display == 1:
    ...
    # set the height/width of each block in the maze
    height = 8
    width = height
    margin = 1
